-- Eliminar tablas si ya existen (en orden correcto por relaciones)
...
-- Tabla profesor con claves foráneas
CREATE TABLE profesor (
  ...
  CONSTRAINT fk_profesor_genero FOREIGN KEY (genero_id) REFERENCES genero (genero_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_profesor_grado FOREIGN KEY (grado_id) REFERENCES grado_academico (grado_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_profesor_estado FOREIGN KEY (estado_id) REFERENCES estado_profesor (estado_id) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Categoría por profesor
CREATE TABLE categoria_profesor (
  ...
  fecha_inicio   DATE,
  fecha_fin      DATE,
  CONSTRAINT fk_catprofesor_profesor FOREIGN KEY (profesor_id) REFERENCES profesor (profesor_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_catprofesor_puesto FOREIGN KEY (puesto_id) REFERENCES puesto (puesto_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_catprofesor_asignatura FOREIGN KEY (asignatura_id) REFERENCES asignatura (asignatura_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT uq_catprofesor UNIQUE (profesor_id, puesto_id, asignatura_id, fecha_inicio)
);

-- Usuarios (admin y visitante)
CREATE TABLE usuario_app (
  ...
  CONSTRAINT fk_usuario_rol FOREIGN KEY (rol_id) REFERENCES rol (rol_id) ON DELETE CASCADE ON UPDATE CASCADE
);
